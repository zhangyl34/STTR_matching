...
class SppBackbone(nn.Module):
    ...
    def forward(self, x: NestedTensor):
        ...
        :return: list containing feature descriptors at different spatial resolution
                ...
                3: [2N, 128, H//16, W//16]
        """
        _, _, h, w = x.left.shape  # N,3,H,W

        src_stereo = torch.cat([x.left, x.right], dim=0)  # (2N,3,H,W)

        # in conv
        output = self.in_conv(src_stereo)  # (2N,32,H/2,W/2)

        # res blocks
        output_1 = self.resblock_1(output)    # (2N,64,H/4,W/4)
        output_2 = self.resblock_2(output_1)  # (2N,128,H/8,W/8)
        ...
        output_3 = torch.cat([spp_1, spp_2, spp_3, spp_4], dim=1)  # (2N,128,H/16,W/16)
