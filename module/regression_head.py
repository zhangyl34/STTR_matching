...
class RegressionHead(nn.Module):
    ...
    def __init__(self, cal: nn.Module, ot: bool = True):
        ...
        self.cal = cal  # build_context_adjustment_layer
        self.ot = ot    # True
    ...
    def _compute_low_res_disp(self, pos_shift: Tensor, attn_weight: Tensor, occ_mask: Tensor):
        ...
        # 得分最高像素的位置
        high_response = torch.argmax(attn_weight, dim=-1)  # (N,h,w)
        ...
        # 最低维前后各打一个 pad
        attn_weight_pad = F.pad(attn_weight, [1, 1], value=0.0)  # (N,h,w,w+2)
        # 用 response_range 提取 attn_weight, pad offset by 1, 
        ...
        # re-normalize to 1
        attn_weight_rw = attn_weight_rw / norm  # (N,h,w,3)
        pos_pad = F.pad(pos_shift, [1, 1]).expand_as(attn_weight_pad)  # (N,h,w,w+2)
        # 窗口的 disparity
        pos_rw = torch.gather(pos_pad, -1, response_range + 1)  # (N,h,w,3)

        # compute low res disparity
        disp_pred_low_res = (attn_weight_rw * pos_rw)  # (N,h,w,3)
    ...
    def _upsample(self, x: NestedTensor, disp_pred: Tensor, occ_pred: Tensor, scale: int):
        ...
        """
        _, _, h, w = x.left.size()  # N, 3, H, W

        # scale disparity
        disp_pred_attn = disp_pred * scale

        # upsample
        disp_pred = F.interpolate(disp_pred_attn[None,], size=(h, w), mode='nearest')  # (1,N,H,W)
        occ_pred = F.interpolate(occ_pred[None,], size=(h, w), mode='nearest')         # (1,N,H,W)
    ...
    def _sinkhorn(self, attn: Tensor, log_mu: Tensor, log_nu: Tensor, iters: int):
        ...
        u, v = torch.zeros_like(log_mu), torch.zeros_like(log_nu)  # (N,h,w+1)
        for idx in range(iters):  # range(10)
    ...
    def _optimal_transport(self, attn: Tensor, iters: int):
        ...
        """
        bs, h, w, _ = attn.shape  # N,h,w,w

        # set marginal to be uniform distribution
        marginal = torch.cat([torch.ones([w]), torch.tensor([w]).float()]) / (2 * w)  # (w+1)
        log_mu = marginal.log().to(attn.device).expand(bs, h, w + 1)  # (N,h,w+1)
        log_nu = marginal.log().to(attn.device).expand(bs, h, w + 1)  # (N,h,w+1)
        ...
        # sinkhorn 循环十次
        attn_ot = self._sinkhorn(similarity_matrix, log_mu, log_nu, iters)  # (N,h,w+1,w+1)
    ...
    def forward(self, attn_weight: Tensor, x: NestedTensor):
        ...
        if x.sampled_cols is not None:
            scale = x.left.size(-1) / float(x.sampled_cols.size(-1))  # 3
        else:
            scale = 1.0
        ...
        if x.disp is not None:  # 输入的视差图，好像没用
        ...
        if x.occ_mask is not None:  # 没有输入
            # handle occlusion
            occ_mask = x.occ_mask
        ...
        # (w)
        pos_shift = self._compute_unscaled_pos_shift(attn_weight.shape[2], attn_weight.device)
        # (1,1,w,w)
        disp_pred_low_res, matched_attn = self._compute_low_res_disp(pos_shift, attn_ot[..., :-1, :-1], occ_mask)
        # (N,h,w), (N,h,w,1)
        # regress low res occlusion
        occ_pred_low_res = self._compute_low_res_occ(matched_attn)  # (N,h,w)
...
def build_regression_head(args):
    ...
    if args.regression_head == 'ot':  # this one
        ot = True
