...
class STTR(nn.Module):
    ...
    def forward(self, x: NestedTensor):
        ...
        feat_right = tokens[bs:]  # (N,128,H,W)

        # downsample
        if x.sampled_cols is not None:
            feat_left = batched_index_select(feat_left, 3, x.sampled_cols)    # (N,128,H,W/3)
            feat_right = batched_index_select(feat_right, 3, x.sampled_cols)  # (N,128,H,W/3)
        if x.sampled_rows is not None:
            feat_left = batched_index_select(feat_left, 2, x.sampled_rows)    # (N,128,H/3,W/3)
            feat_right = batched_index_select(feat_right, 2, x.sampled_rows)  # (N,128,H/3,W/3)

        # transformer
        attn_weight = self.transformer(feat_left, feat_right, pos_enc)  # (N,H/3,W/3,W/3)
