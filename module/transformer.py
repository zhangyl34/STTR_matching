...
class Transformer(nn.Module):
    ...
    def __init__(self, hidden_dim: int = 128, nhead: int = 8, num_attn_layers: int = 6):
        ...
        self.nhead = nhead            # 8
        self.num_attn_layers = num_attn_layers  # 6
    ...
    def _alternating_attn(self, feat: torch.Tensor, pos_enc: torch.Tensor, pos_indexes: Tensor, hn: int):
        ...
        for idx, (self_attn, cross_attn) in enumerate(zip(self.self_attn_layers, self.cross_attn_layers)):
            ...
            if idx == self.num_attn_layers - 1:  # 5
    ...
    def forward(self, feat_left: torch.Tensor, feat_right: torch.Tensor, pos_enc: Optional[Tensor] = None):
        ...
        :param pos_enc: relative positional encoding, [2w-1,C]
        :return: cross attention values [N,H,W,W], dim=2 is left image, dim=3 is right image
        """

        bs, c, hn, w = feat_left.shape  # N,128,H/3,W/3
        # (128,W/3,H/3,N) -> (128,W/3,HN/3) -> (W/3,HN/3,128)
        ...
        if pos_enc is not None:
            with torch.no_grad():
                ...
                indexes_r = torch.linspace(w - 1, 0, w).view(w, 1).to(feat_left.device)  # [w-1;...;0]
                indexes_c = torch.linspace(0, w - 1, w).view(1, w).to(feat_left.device)  # [0,...,w-1]
                pos_indexes = (indexes_r + indexes_c).view(-1).long()  #(w*w)
        else:
            pos_indexes = None

        # concatenate left and right features
        feat = torch.cat([feat_left, feat_right], dim=1)  # (W/3,2HN/3,128)

        # compute attention
        attn_weight = self._alternating_attn(feat, pos_enc, pos_indexes, hn)
        # (hN,w,w) -> (N,h,w,w) dim=2 left image, dim=3 right image
...
class TransformerSelfAttnLayer(nn.Module):
    ...
    def forward(self, feat: Tensor,
        ...
        """
        feat2 = self.norm1(feat)  # (W/3,2HN/3,128)
...
class TransformerCrossAttnLayer(nn.Module):
    ...
    def forward(self, feat_left: Tensor, feat_right: Tensor,
        ...
        """
        feat_left_2 = self.norm1(feat_left)    # (W/3,HN/3,128)
        feat_right_2 = self.norm1(feat_right)  # (W/3,HN/3,128)
        ...
        # concat features
        feat = torch.cat([feat_left, feat_right], dim=1)  # (w,2hN,128)
